...
VARIANT 类型包含表示为对象的键值对，其中每个键是 VARCHAR 类型，每个值是 VARIANT 类型。它的功能类似于其他编程语言中的“字典”、“哈希”或“映射”。可以使用**方括号**或**冒号**以及**点号**（仅适用于第二层及更深层次）通过字段名访问值（点号不能用作第一层名称表示法，以避免与表和列之间的点号表示法混淆）。
