Understood. Please provide the content you'd like me to translate, and I will follow the rules and guidelines you've outlined.
...
`PARTITION BY` 子句允许将行分组为子组，例如按城市、按年份等。`PARTITION BY` 子句是可选的。您可以在不将行分组为子组的情况下分析整个行组。
...
在此示例中，我们使用 RANK() 函数按薪水降序对员工进行排名。薪水最高的员工将排名为1，薪水最低的员工将排名最高。
```sql
...
在此示例中，我们使用 SUM() 函数和 PARTITION BY 来计算每个部门支付的总薪水。每行将显示部门及其总薪水。
```sql
...
在此示例中，我们使用 SUM() 函数和累积窗口框架来计算每个部门内的薪水累计总和。累计总和基于按 employee_id 排序的员工的薪水。
```sql
