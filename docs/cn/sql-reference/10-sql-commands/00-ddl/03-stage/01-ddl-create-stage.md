...
<TabItem value="Amazon S3-compatible Storage" label="Amazon S3-like 存储服务">
...
:::note
要在Amazon S3上创建外部Stage，您还可以使用IAM用户账户，使您能够为Stage定义细粒度的访问控制，包括指定对特定S3存储桶的读或写访问权限。请参见[示例3：使用AWS IAM用户创建外部Stage](#example-3-create-external-stage-with-aws-iam-user)。
:::
</TabItem>
...
<TabItem value="Tencent Cloud Object Storage" label="Tencent Cloud 对象存储">
...
有关访问Tencent Cloud对象存储的连接参数，请参见[连接参数](/00-sql-reference/51-connect-parameters.md)。
</TabItem>
...
详情请参见[输入 & 输出文件格式](../../../00-sql-reference/50-file-format-options.md)。
