...
在此示例中，使用 INSERT OVERWRITE 语句截断 employee 表并插入新数据，替换所有现有记录为 ID 为 100 的员工的值。
...
```sql
-- 创建名为 'employee_info' 的表，包含三列：'employee_id'、'employee_name' 和 'department'
...
-- 创建名为 'employee_data' 的表，包含三列：'ID'、'Name' 和 'Dept'
...
-- 从 'employee_info' 向 'employee_data' 插入数据
...
此示例演示了创建一个名为 "sales_summary" 的汇总表，用于存储从 sales 表聚合的销售数据，如每个产品的总销售数量和收入：
...
```sql
-- 创建 'staff_records' 表，包含 'employee_id'、'department'、'salary' 和 'status' 列，并设置默认值
...
我们将创建一个名为 `mystage` 的内部 stage，然后向其填充示例数据。
